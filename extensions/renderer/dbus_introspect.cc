...
#define fprintf(out, fmt, args...) do {} while(0)
...
void ParserPrint(Parser *parser) {
  ...
  for (interface_ite = root_node->interfaces_.begin();
    ...
    std::list<BusProperty*>::iterator pro_ite;
    for(pro_ite = interface->properties_.begin();
        pro_ite != interface->properties_.end();
        pro_ite++) {
      BusProperty* property = *pro_ite;
...
static void expat_StartElementHandler(void *userData, 
  ...
  if (!strcmp(name, "node")) {
    ...
    BusProperty * property = new BusProperty;
    parser->is_on_property = true;
    parser->current_property = property;
    
    property->name_ = GetAttribute(attrs, "name"); 
    property->type_ = GetAttribute(attrs, "type");
    //property->access_ 
  } else {
...
static void expat_EndElementHandler(void *userData, 
  ...
  if (!strcmp(name, "node")) { 
    ...
    parser->is_on_property = false;
    parser->current_property = NULL; 
  } else {
